...
namespace BiopacInterop
{
	...
	BiopacCommunicator::BiopacCommunicator(const bool& _isSyncOnly)
        : isSyncOnly(_isSyncOnly)
	{
	}


    bool BiopacCommunicator::isSynchOnly() const
    {
        return isSyncOnly;
    }
	...
	*/
	...
	int BiopacCommunicator::startTcpServer()
	{
		...
		unsigned int analogDataSize = 100 * analogChannels.size();
		unsigned int digitalDataSize = 100 * digitalChannels.size();
		unsigned int calcDataSize = 100 * calcChannels.size();
		...
		while (true)
		{
			...
			for (unsigned int s = 0; s < analogChannels.size(); s++)
			{
				...
				if (iResult > 0)
				{
                    ...
                    std::cout << "TCP SERVER: recv analogClientSockets: " << s << std::endl;
				}
				else if (iResult == 0)
				{
...
*/
    ...
    int BiopacCommunicator::getAnalogData(const int& index)
    {
        if (index < analogData.size() && !analogData.empty())
            return analogData.at(index);
        return (-1);
    }

    int BiopacCommunicator::getDigitalData(const int& index)
    {
        if (index < digitalData.size() && !digitalData.empty())
            return digitalData.at(index);
        return (-1);
    }

    double BiopacCommunicator::getCalcData(const int& index)
    {
        if (index < calcData.size() && !calcData.empty())
            return calcData.at(index);
        return (-1);
    }

    double BiopacCommunicator::getDataTimeData(const int& index)
    {
        if (index < dataTime.size() && !dataTime.empty())
