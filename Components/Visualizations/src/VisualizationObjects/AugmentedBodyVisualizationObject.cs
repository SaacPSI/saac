...
namespace SAAC.Visualizations
{
    ...
    public class AugmentedBodyVisualizationObject : ModelVisual3DValueVisualizationObject<AzureKinectBody>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AugmentedBodyVisualizationObject"/> class.
        /// </summary>
        public AugmentedBodyVisualizationObject()
        {
            this.Skeleton = new AugmentedSkeletonVisualizationObject<JointId>(
                nodeVisibilityFunc:
                    jointType =>
                    {
                        var jointState = this.CurrentData.Joints[jointType].Item2;
                ...
                edgeVisibilityFunc:
                    bone =>
                    {
                        var parentState = this.CurrentData.Joints[bone.Item1].Item2;
                        var childState = this.CurrentData.Joints[bone.Item2].Item2;
        ...
        private void UpdateBillboard()
        {
            if (this.CurrentData != null)
            {
                ...
                this.Billboard.SetCurrentValue(this.SynthesizeMessage(Tuple.Create(pos, $"Body{this.CurrentData.TrackingId}")));
            }
        }
